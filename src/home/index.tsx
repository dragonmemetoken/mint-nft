...
const Home = () => {
    ...
    const [shownNFT1, setShownNFT1] = useState(Math.floor(Math.random() * nftsLength));
    ...
    return (
        <C.Home className="tw-bg-[#ffac4a!important] tw-pb-[100px]">
            <C.Container>
                <C.Header>
                    <div className="">
                        <img src={publicUrl + '/images/dragonmeme.png'} className="tw-w-[60px]" />
                    </div>
                ...
                <C.Launch showMintedNfts={showMintedNfts ? "true" : "false"} className="tw-bg-[#ffe9c9!important] tw-text-[#000000]" style={{background: "initial", boxShadow: "initial"}}>
                    ...
                    {!loading && (
                        <>
                                ...
                                <>
                                    <C.LaunchInfo>
                                        ...
                                        <C.TotalMinted>
                                            ...
                                            <C.TotalMintedProgress value={Math.floor((collection.mintedSupply / collection.supply * 100) * 100) / 100} className="tw-bg-[#ff7000!important]"></C.TotalMintedProgress>
                                        </C.TotalMinted>

                                        <C.Description className="tw-text-[#000000!important]">{config.description}</C.Description>

                                        {(config.website || config.twitter || config.discord) && (
                                            <C.Links>
                                                {config.website &&
                                                    <C.Link href={config.website} target="_blank" rel="noreferrer">
                                                        <FontAwesomeIcon icon={faGlobe} className="tw-text-[#ff7000]" />
                                                    </C.Link>
                                                }
                                                {config.twitter &&
                                                    <C.Link href={config.twitter} target="_blank" rel="noreferrer">
                                                        <FontAwesomeIcon icon={faTwitter} className="tw-text-[#ff7000]" />
                                                    </C.Link>
                                                }
                                                {config.discord &&
                                                    <C.Link href={config.discord} target="_blank" rel="noreferrer">
                                                        <FontAwesomeIcon icon={faDiscord} className="tw-text-[#ff7000]" />
                                                    </C.Link>
                                                }
                                                <C.Link href="https://t.me/DragonMeme2024" target="_blank" rel="noreferrer">
                                                    <FontAwesomeIcon icon={faTelegram} className="tw-text-[#ff7000]" />
                                                </C.Link>
                                            </C.Links>
                                        )}
                                        ...
                                        <C.Phases>
                                            {phases.map((phase, index) => (
                                                <C.Phase key={index} active={currentPhase.name === phase.name ? "true" : "false"} switch={!(!phase.noend && new Date(phase.end_time) < new Date()) ? "true" : "false"} onClick={() => switchPhase(phase)} className="tw-bg-[#ff7000!important]">
                                                    <C.PhaseTop>
                                                        <C.PhaseTitle className="tw-text-[#ffffff!important]">Free Minting</C.PhaseTitle>
                                                        {!phase.noend && (
                                                            <>
                                                    ...
                                                    <C.PhaseBottom>
                                                        {'1 Per Wallet •'} {new BigNumber(phase.unit_price).div(1e6).toString()} SEI
                                                    </C.PhaseBottom>
                                    ...
                                    <C.LaunchMint>
                                        ...
                                        <C.Image>
                                            <div className="tw-relative">
                                                {nfts.map((nft, index) => (
                                                    <div key={index} className="tw-absolute tw-top-[0] tw-left-[0]">
                                                        <img src={nft} className={'tw-w-[100%] ' + ((shownNFT1 === index) ? 'tw-opacity-[1]' : 'tw-opacity-[0]')} style={{transition:'500ms'}} alt="Dragon Meme" />
                                                    </div>
                                                ))}
                                        ...
                                        <C.MintInfo>
                                            <C.Price className="tw-text-[#000000!important]">
                                                Price: <span className="tw-text-[#000000!important]">{new BigNumber(currentPhase.unit_price).div(1e6).times(amount).toString()} SEI</span>
                                            </C.Price>
                                            {/*<C.Amount className="tw-bg-[#ff7000!important]">*/}
                                            {/*    <C.AmountButton onClick={decrementAmount} className="tw-bg-[#ffe9c9!important] tw-text-[#000000!important] tw-font-[500]">*/}
                                            {/*        &minus;*/}
                                            {/*    </C.AmountButton>*/}
                                            {/*    <C.AmountValue ref={amountInput} type="number" step="1" min={1} defaultValue={1} onChange={onAmountChange} className="tw-font-[700]" />*/}
                                            {/*    <C.AmountButton onClick={incrementAmount} className="tw-bg-[#ffe9c9!important] tw-text-[#000000!important] tw-font-[500]">*/}
                                            {/*        &#43;*/}
                                            {/*    </C.AmountButton>*/}
                                            {/*</C.Amount>*/}
                                        </C.MintInfo>
                                        <C.MintButton onClick={mint} disabled={walletWhitelisted === false || collection.supply - collection.mintedSupply <= 0} className="tw-bg-[#ff7000!important] tw-text-[#ffffff!important] hover:tw-bg-[#cc5800!important]">
                                        ...
                                        {myMintedNfts.length > 0 && (
                                            <C.MintedBalance onClick={() => loadMinted()} className="tw-text-[#000000!important]">
                            ...
                            {showMintedNfts && (
                                <C.MintedNfts>
                                    <C.MintedNftsHeader>
                                        <C.GoBack onClick={() => setShowMintedNfts(false)} className="tw-bg-[#ff7000!important] tw-text-[#ffffff!important] hover:tw-bg-[#cc5800!important]">Back</C.GoBack>
                                    </C.MintedNftsHeader>
                                    <C.MintedNftsBody>
                                        {myMintedNftsData.map((mint: any, i: any) => (
                                            <C.Nft key={i} className="tw-bg-[#ffe9c9!important]" style={{border: "1px solid #B9A382"}}>
